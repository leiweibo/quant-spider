...
2. pipenv 安装完成之后，执行`pipenv shell` 进入到pipenv虚拟环境中，然后执行`pipenv sync` 安装依赖

    __注意： 这里需要修改一下 Pipfile.lock里面taos的依赖，我写的是本地的路径，暂时先手动去修改，暂时我本地的python是3.6版本，如果版本不一致需要修改一下里面的python version__

3. tdengine数据库，执行quant.sql文件里面的脚本
